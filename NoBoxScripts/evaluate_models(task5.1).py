...
def evaluate_model_with_aug(model_path, image_dir, mask_dir, val_names, device):
    model = deeplabv3_resnet50(num_classes=2).to(device)
    model.load_state_dict(torch.load(model_path, map_location=device))
    model.eval()
...
def main():
    ...
    models = {
        ...
        "Trimap": ("best_deeplab_trimap.pth", "oxford-iiit-pet/annotations/trimaps"),
        "With Aug": ("best_deeplab_with_aug.pth", "oxford-iiit-pet/annotations/trimaps"),
    }
    ...
    result_path = "evaluation_results.txt"
