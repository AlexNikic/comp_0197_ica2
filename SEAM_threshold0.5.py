...
#############################################
def save_mask_from_cam(cam_pil, save_path, threshold=0.5):
...
#############################################
def process_remaining_missing_masks(model, image_dir, mask_dir, device, threshold=0.5):
    all_images = set(f.replace(".jpg", "") for f in os.listdir(image_dir) if f.endswith(".jpg"))
    all_masks = set(f.replace(".png", "") for f in os.listdir(mask_dir) if f.endswith(".png"))
...
if __name__ == "__main__":
    ...
    os.makedirs("cam_vis_thr05", exist_ok=True)
    os.makedirs("pseudo_mask_thr05", exist_ok=True)
    ...
    for image, label, filename in dataloader:
        ...
        overlay_path = f"cam_vis_thr05/{filename}_cam_overlay.jpg"
        if not os.path.exists(overlay_path):
            overlay = overlay_cam_on_image(image, cam_pil)
            overlay.save(overlay_path)
        mask_path = f"pseudo_mask_thr05/{filename.replace('.jpg', '.png')}"
