...
#############################################
def save_mask_from_cam(cam_pil, save_path, threshold=0.8):
...
#############################################
def process_unlabeled_images(model, image_dir, mask_dir, device, threshold=0.8):
...
#############################################
def process_remaining_missing_masks(model, image_dir, mask_dir, device, threshold=0.8):
    all_images = set(f.replace(".jpg", "") for f in os.listdir(image_dir) if f.endswith(".jpg"))
    all_masks = set(f.replace(".png", "") for f in os.listdir(mask_dir) if f.endswith(".png"))
...
if __name__ == "__main__":
    ...
    os.makedirs("cam_vis_thr08", exist_ok=True)
    os.makedirs("pseudo_mask_thr08", exist_ok=True)

    # Load model
    model = ResNetMultiTask(num_breeds=37)
    model_path = os.path.join("NoBoxScripts", "best_resnet_multitask_model.pth")
    ...
    for image, label, filename in dataloader:
        ...
        overlay_path = f"cam_vis_thr08/{filename}_cam_overlay.jpg"
        if not os.path.exists(overlay_path):
            overlay = overlay_cam_on_image(image, cam_pil)
            overlay.save(overlay_path)
        mask_path = f"pseudo_mask_thr08/{filename.replace('.jpg', '.png')}"
        if not os.path.exists(mask_path):
            save_mask_from_cam(cam_pil, mask_path, threshold=0.8)

    # Handle additional unlabeled images
    process_unlabeled_images(model, "images", "pseudo_mask_thr08", device, threshold=0.8)
    process_remaining_missing_masks(model, "images", "pseudo_mask_thr08", device, threshold=0.8)

    # === Step 7: Output list of unlabeled data ===
    def export_unlabeled_image_list(image_dir, list_path, save_path="unlabeled_images_thr08.txt"):
