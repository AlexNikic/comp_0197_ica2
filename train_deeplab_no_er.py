...
def equivariant_cross_loss(pred1, pred2, target, weight=1.0):
...
def train_model(train_fraction=0.01, val_fraction=0.01):  
    ...
    for epoch in range(1, 11):  
        ...
        for images, masks in train_loader:
            ...
            # Compute the loss with only the equivariant cross-regularisation
            loss = equivariant_cross_loss(outputs, augmented_outputs, masks)
        ...
        if mean_iou > best_mIoU:
            best_mIoU = mean_iou
            torch.save(model.state_dict(), os.path.join(save_dir, "deeplab_no_er.pth"))
